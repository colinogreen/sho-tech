...
{  
    ...
    getAlertMessages()
    {
        ...
        const days_since_update_round = Math.floor(days_since_update);
        if(Number.isInteger(days_since_update_round) && days_since_update_round > 0){
            let day_string = (days_since_update_round === 1)? "day": "days";
            messages.days_since_update = ["It has been at least " + days_since_update_round + " " + day_string + " since the last data update", new Date().toDateString(), days_since_update_round];
        }
        else       
        {
            ...
            {
                messages.days_since_update = ["Last data update: " + new Date(this.result_data.last_modified), new Date().toDateString(), days_since_update_round];
            }
        }
    ...
    getSixIndividualMonthsData(object_label)
    {
        ...
        for(var i = 1; i < 7; i++)
        {
            ...
            //** Important! In addition to setting the month, set the day to 1st of the month to ...
